...
publishedAt: 2026-08-04T07:46:27.304Z
author: 今夜白
tags:
  - 论文
  - Sparse Attention
...
language: zh-CN
agent:
  summary: 本文解读 ReTopK：为每个 Attention Head 缓存历史 Query 与 Top-K 支持集合，通过相似 Query 召回、最近窗口保护和候选集精确重排，减少长上下文动态稀疏注意力的完整 QK 扫描与全局 Top-K 成本；同时分析其 GPU Kernel、质量—性能权衡、系统边界及其对分层 KV 预取的启发。
...
默认 `R=4`、`K=512`、`W=32` 时，理论上限为 2080；由于历史支持之间存在大量重叠，64K 实验中的平均候选数约为 701。
